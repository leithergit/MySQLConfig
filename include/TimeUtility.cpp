...
void	StringFromSystemTimeA(CHAR* szTimeString,int nSize,bool bMillionSecond)
{
	SYSTEMTIME sysTime;
	GetLocalTime(&sysTime);
	if (bMillionSecond)
	{
		sprintf_s(szTimeString,nSize,"%04d-%02d-%02d %02d:%02d:%02d.%03d",sysTime.wYear,sysTime.wMonth,sysTime.wDay,sysTime.wHour,sysTime.wMinute,sysTime.wSecond,sysTime.wMilliseconds);
	}
	else
	{
		sprintf_s(szTimeString,nSize,"%04d-%02d-%02d %02d:%02d:%02d",sysTime.wYear,sysTime.wMonth,sysTime.wDay,sysTime.wHour,sysTime.wMinute,sysTime.wSecond);
	}
}

void	StringFromSystemTimeW(WCHAR* szTimeString,int nSize,bool bMillionSecond)
{
	SYSTEMTIME sysTime;
	GetLocalTime(&sysTime);
	if (bMillionSecond)
	{
		swprintf_s(szTimeString,nSize,L"%04d-%02d-%02d %02d:%02d:%02d.%03d",sysTime.wYear,sysTime.wMonth,sysTime.wDay,sysTime.wHour,sysTime.wMinute,sysTime.wSecond,sysTime.wMilliseconds);
	}
	else
	{
		swprintf_s(szTimeString,nSize,L"%04d-%02d-%02d %02d:%02d:%02d",sysTime.wYear,sysTime.wMonth,sysTime.wDay,sysTime.wHour,sysTime.wMinute,sysTime.wSecond);
	}
}
...
//闰二月的秒数
#define		__Leap_Feb_Sec                 2505600
//二月的秒数
#define		__Feb_Sec						2419200
//闰年的秒数
		...
		case 2:
			if (bLeapYear)
			{
				if (pSystemTime->wDay >29)
					return FALSE;
				nMonthTime += __Leap_Feb_Sec;
			}
			else
			{
				if (pSystemTime->wDay >28)
					return FALSE;
				nMonthTime += __Feb_Sec;
			}
			break;		
		case 4:			
		case 6:
