...
void CRunlogA::RunlogBin(LPCSTR szTitle,unsigned char *pBinBuff,int nLen,CHAR chSeperator)
{
	if (!g_bEnableRunlog)
	{
		return ;
...
void CRunlogA::RunlogHuge(unsigned char *szHugeData,int nDataLen,LPCSTR pFormat,...)
{
	if (!g_bEnableRunlog)
	{
		return ;
...
void CRunlogW::RunlogBin(LPCWSTR szTitle,unsigned char *pBinBuff,int nLen,WCHAR chSeperator)
{
	if (!g_bEnableRunlog)
	{
		return ;
...
void CRunlogW::RunlogHuge(unsigned char *szHugeData,int nDataLen,LPCWSTR pFormat,...)
{
	if (!g_bEnableRunlog)
	{
		return ;
