...
int AnalysisDataW(WCHAR *szSrc,int nSrcLen,WCHAR *szText,int &nTextLen,WCHAR Flag)
{
	if (nSrcLen >0)
	{
		...
		nTextLen = i;
		memcpy(szText,szSrc,nTextLen*sizeof(WCHAR));
		return (nSrcLen - nTextLen);
	}
	else
		return 0;
...
BOOL IsProcessElevated()
{
	...
	DWORD retSize = 0;
	if (!GetTokenInformation(hToken, TokenElevation, &elevationInfo, sizeof(elevationInfo), &retSize))
	{
		errCode = GetLastError();
		throw errCode;
	}
...
#include <string>
int RunWinProcess(LPCSTR szProcess, LPCSTR szArgument, LPCSTR szWorkDirectory)
{
	SECURITY_ATTRIBUTES sa;
	HANDLE hStdOutput;			// 命令行输出管道,用于接收命令行的输出信息
	HANDLE hReadStdOutput;		// 读取命令行输出管道，用于读取命令行的输出信息

	HANDLE hWriteStdInput;		// 向命令行写入控制信息的管道
	HANDLE hStdInput;			// 命令接收信息的管道
	sa.nLength = sizeof(SECURITY_ATTRIBUTES);
	sa.lpSecurityDescriptor = NULL;
	sa.bInheritHandle = TRUE;
	if (!CreatePipe(&hReadStdOutput, &hStdOutput, &sa, 4096) ||
		!CreatePipe(&hStdInput, &hWriteStdInput, &sa, 4096))
	{
		return GetLastError();
	}
	CHAR szShellCmd[1024] = { 0 };
	CHAR szCommandLine[1024] = { 0 };
	if (!GetEnvironmentVariableA("ComSpec", szShellCmd, 1024))
	{
		return GetLastError();
	}
	...
	try
	{
		...
		//sprintf(szCommandLine, _T("/c \"%s\" %s"), szProcess,szArgument);
		//cout << szCommandLine << endl;
		//if (!CreateProcess(szShellCmd, (TCHAR *)szCommandLine, NULL, NULL, TRUE, NULL, NULL, NULL, &si, &pi))
		//	return GetLastError();
		if (!CreateProcessA(szProcess, (CHAR*)szArgument, NULL, NULL, TRUE, NULL, NULL, szWorkDirectory, &si, &pi))
			return GetLastError();
		...
		while (true)	//等待控制台进程关闭
		{
			ZeroMemory(szBuffer, 1024);
			dwBytesRead = 0;
			PeekNamedPipe(hReadStdOutput, szBuffer, 1024, &dwBytesRead, 0, 0);	//管道是否有数据可读
			if (dwBytesRead)
			{
				if (!ReadFile(hReadStdOutput, (void*)szBuffer, 1024, &dwNumRead, nullptr))
					break;
				strConsole += szBuffer;
				Overlapped.Offset += dwNumRead;
				Sleep(50);
			}
			else
				break;
