...
struct CSockClient:public ListNode
{
public:
	SOCKET		s;
...
// 		LeaveCriticalSection(&m_csRecvList);
	...
	int ConnectServer(LPCTSTR szServerIP, WORD nPort, int nNetTimeout)
	{
		...
		__try
		{
			...
			//接收缓冲区
			int nRecvBuf = 16*1024;//设置为1024 byte
	...
	int AsyncConnect(const TCHAR *szServer, int nPort, int nLocalPort = -1)
	{
		int nResult = SOCKET_ERROR;
		__try
		{
			...
			int nNonBlock = 1;
			if (ioctlsocket(s, FIONBIO, (unsigned long*)&nNonBlock) !=0)
				__leave;
	...
	int CreateSocket(SOCKET &sClient)
	{
		...
		__try
		{
			...
			//和接收数据量比较大,可以设置socket缓冲区,而避免了send(),recv()不断的循环收发：
			//接收缓冲区
			//int nRecvBuf = 16384;	//设置为8192 byte
...
// 			}
	...
	int Recv()
	{
		...
		int nRecvNum = recv(s, (char *)&pRecvBuffer[nDataLength], MAX_BUFFER_LEN - nDataLength, 0);
		tLastAction = GetPerfTime();
		if (nRecvNum == 0)
		{//连接关闭
		...
		TraceMsg(_T("%s Recvnum = %d.\n"), __FUNCTIONW__, nRecvNum);
		nDataLength += nRecvNum;
		//MemMerge(&pRecvBuffer, nDataLength, nRecvBufferSize, szTemp, nRecvNum);
		//} while (true);
...
public:
	CInitializeWinSock()
	{
		...
		else
		{
			...
			MessageBox(GetDesktopWindow(), szError, _T("Initialize Winsock2.2"), MB_OK | MB_ICONSTOP);
			exit(0);
		}
	}
};
...
{
public:
	ThreadInfo*		m_pThreadPool = nullptr;
	CRunlog*		m_pRunlog = nullptr;
	int				m_nMaxThreads = 1;
	MyList			*m_pClientList;
	CClientMonitor(int nMaxThreads = __MaxThread, CRunlog *pRunlog = nullptr)
	{
		m_pClientList = new MyList();
		m_nMaxThreads = nMaxThreads;
		m_pThreadPool = new ThreadInfo[nMaxThreads];
	}
	~CClientMonitor()
	{
		Stop();
	...
	bool Start()
	{
		...
		__try
		{
			...
			for (int i = 0; i < m_nMaxThreads; i++)
			{
				...
				m_pThreadPool[i].pThis = this;
				m_pThreadPool[i].nIndex = i;
				m_pThreadPool[i].bRun = true;
				m_pThreadPool[i].dwEventCount = 0;
				m_pThreadPool[i].hThread = (HANDLE)_beginthreadex(NULL, 0, MonitorThread, &m_pThreadPool[i], CREATE_SUSPENDED, 0);
			...
			if (m_pThreadPool[0].hThread)
				ResumeThread(m_pThreadPool[0].hThread);
			bSucceed = true;
		}
		__finally
		{
	...
	void Stop()
	{
		...
		for (int i = 0; i < m_nMaxThreads; i++)
		{
			ThreadInfo &pThread = m_pThreadPool[i];
			if (pThread.hThread)
			{
				pThread.bRun = false;
	...
	static UINT __stdcall MonitorThread(void *p)
	{
		...
		int &nThreadIndex = pThreadInfo->nIndex;
		CClientMonitor *pThis = (CClientMonitor *)pThreadInfo->pThis;
			...
			if (pThreadInfo->listClientstoRemove.size())
			{
				for (auto it = pThreadInfo->listClientstoRemove.begin(); it != pThreadInfo->listClientstoRemove.end();)
				{
					for (int k = 0; k < pThreadInfo->dwEventCount; k++)
					{
						...
						{
							pThis->RemoveClient(pThreadInfo, k);
							break;
						}
					}
	...
	int ProcessEvent(LPThreadInfo pThreadInfo, DWORD dwIndex)
	{
		...
		if (NetworkEvents.lNetworkEvents & FD_READ)
		{
			...
			{
				TraceMsgA("%s FD_READ Error Code:%d.\n", __FUNCTION__, NetworkEvents.iErrorCode[FD_READ_BIT]);
			}
		...
		{// send succeed
			...
			{
				TraceMsgA("%s FD_WRITE Error Code:%d.\n", __FUNCTION__, NetworkEvents.iErrorCode[FD_WRITE_BIT]);
			}
		...
		{// connect succeed
			...
			{
				TraceMsgA("%s FD_READ Error Code:%d.\n", __FUNCTION__, NetworkEvents.iErrorCode[FD_CONNECT_BIT]);
			}
		...
		{// socket is closed
			...
			{
				TraceMsgA("%s FD_READ Error Code:%d.\n", __FUNCTION__, NetworkEvents.iErrorCode[FD_CLOSE_BIT]);
			}
			return RemoveClient(pThreadInfo, dwIndex);
		}
		return 0;
	}
	virtual int OnAccept(CSockClient *pClient)
	{
		return 0;
	}
	...
	virtual void CheckFreeClient(LPThreadInfo pThreadInfo)
	{
		return ;
	}

	bool AddClient(CSockClient *pClient)
	{
		bool bResumeThread = false;
		bool bAdapterThread = false;
		for (int nIndex = 0; nIndex < m_nMaxThreads; nIndex++)
		{
			ThreadInfo &TI = m_pThreadPool[nIndex];
			EnterCriticalSection(&TI.csThread);
			if (TI.dwEventCount < __MaxClients)
			{
				TI.pClientArray[TI.dwEventCount] = pClient;
				TI.hEventArray[TI.dwEventCount] = pClient->hSocketEvent;
				TI.dwEventCount++;
				TraceMsg(_T("%s MonitorThread[%d] Clients:%d.\r\n"),__FUNCTIONW__, nIndex, TI.dwEventCount);
				if (TI.dwEventCount == 1 &&	// 新添加的第一个事件,则唤醒该线程
					!TI.bResumed)
				{
					bResumeThread = true;
				}
			...
			LeaveCriticalSection(&TI.csThread);
			if (bResumeThread)
			{
				TraceMsg(_T("%s MonitorThread[%d] is Resumed.\r\n"), __FUNCTIONW__, nIndex);
	...
	// Remove a client from a Thread indicated by pThreadInfo
	virtual int RemoveClient(LPThreadInfo pThreadInfo, DWORD dwIndex)
	{
		WSAEVENT hClientEvent = pThreadInfo->hEventArray[dwIndex];
		CSockClient *pClient = pThreadInfo->pClientArray[dwIndex];
		::EnterCriticalSection(&pThreadInfo->csThread);
	...
	virtual void RemoveClient(CSockClient *pClient)
	{
		bool bSuspendThread = false;
		bool bMatched = false;
		for (int nIndex = 0; nIndex < m_nMaxThreads; nIndex++)
		{
			ThreadInfo &TI = m_pThreadPool[nIndex];
			...
			int nClient = 0;
			EnterCriticalSection(&TI.csThread);
			for (; nClient < TI.dwEventCount; nClient++)
			{
				if (TI.pClientArray[nClient] == pClient)
				{
					bMatched = true;
					break;
				}
			...
			{
				TI.dwEventCount--;
				memmove(&TI.hEventArray[nClient], &TI.hEventArray[nClient + 1], (TI.dwEventCount - nClient)*sizeof(WSAEVENT));
				TI.hEventArray[TI.dwEventCount] = NULL;
				memmove(&TI.pClientArray[nClient], &TI.pClientArray[nClient + 1], (TI.dwEventCount - nClient)*sizeof(CSockClient*));
				TI.pClientArray[TI.dwEventCount] = NULL;
				if (!TI.dwEventCount)
					bSuspendThread = true;
			}
			LeaveCriticalSection(&TI.csThread);
		...
		if (m_pClientList)
		{
			m_pClientList->Remove(pClient);
			delete pClient;
		}
	}
};

class CSocketServer :public CClientMonitor
{
public:
	CRunlog*			m_pRunlog;
...
public:
	...
	virtual void Stop()
	{
		...
		if (m_pClientList)
		{
			ListNode *pHeader = m_pClientList->Header();
			while (pHeader)
			{
				CSockClient *pTemp = (CSockClient *)pHeader;
				pHeader = pHeader->pNext;
				DisConnect(pTemp->s);
				delete pTemp;
			}
	...
	virtual int RemoveClient(LPThreadInfo pThreadInfo,DWORD dwIndex)
	{	
		__super::RemoveClient(pThreadInfo, dwIndex);
		CSockClient *pClient = pThreadInfo->pClientArray[dwIndex];
		DisConnect(pClient->s);
		OnDisConnect(pClient);
		//if (m_sServer != INVALID_SOCKET)
		__super::RemoveClient(pClient);
		return 0;
	}
};
