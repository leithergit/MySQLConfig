...
IMPLEMENT_DYNAMIC(CHeaderComboBox, CComboBox)

CHeaderComboBox::CHeaderComboBox()
{
	m_pHeaderCtrl = NULL;
	m_nCurItem = -1;
}
...
void CHeaderComboBox::SetHeaderCtrl(CHeaderCtrl* pHeaderCtrl)
{
	if (pHeaderCtrl && IsWindow(pHeaderCtrl->m_hWnd))
		m_pHeaderCtrl = pHeaderCtrl;
}

BEGIN_MESSAGE_MAP(CHeaderComboBox, CComboBox)
	ON_WM_KILLFOCUS()
	ON_WM_CREATE()
	ON_CONTROL_REFLECT(CBN_KILLFOCUS, &CHeaderComboBox::OnCbnKillfocus)
	ON_CONTROL_REFLECT(CBN_CLOSEUP, &CHeaderComboBox::OnCbnCloseup)
	//ON_CONTROL_REFLECT(CBN_SELENDOK, &CListComboBox::OnCbnSelendOK)
END_MESSAGE_MAP()

// CListComboBox message handlers

void CHeaderComboBox::OnKillFocus(CWnd* pNewWnd)
{
	CComboBox::OnKillFocus(pNewWnd);
	if (m_nCurItem < 0 )
		return;
	...
	CWnd *pList = m_pHeaderCtrl->GetParent();
	WORD nListID = (WORD)pList->GetDlgCtrlID();
	WORD nHeaderCtrlID = (WORD)GetDlgCtrlID();
	WORD nItem = m_nCurItem;
	pList->GetParent()->SendMessage(WM_HEADERCTRLS_KILLFOCUS, MAKEWPARAM(nHeaderCtrlID, nListID), MAKELPARAM(0, nItem));
	ShowWindow(SW_HIDE);
}

int CHeaderComboBox::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	if (CComboBox::OnCreate(lpCreateStruct) == -1)
		return -1;
...
void CHeaderComboBox::OnCbnKillfocus()
{

}

void CHeaderComboBox::OnCbnCloseup()
{
	if (!m_pHeaderCtrl)
		return;
...
void CHeaderComboBox::OnCbnSelendOK()
{
	if (m_nCurItem < 0)
		return;
	if (!m_pHeaderCtrl)
...
BOOL CHeaderComboBox::PreTranslateMessage(MSG* pMsg)
{
	if (pMsg->message == WM_KEYDOWN && pMsg->wParam == VK_RETURN)
	{
		GetParent()->SetFocus();
